<fieldset>
	...
	<dl{if $errorType.messagingAlias|isset} class="formError"{/if}>
		...
		<dd>
			<input type="text" id="messagingAlias" name="messagingAlias" {if $group|isset}value="{$group->messagingAlias}"{/if} class="long" />
	...
	<dl>
		<dt></dt>
		<dd>
			<label><input type="checkbox" id="canBeMessaged" name="canBeMessaged" value="1" {if $group|isset && $group->canBeMessaged}checked="checked" {/if} /> {lang}wcf.acp.group.canBeMessaged{/lang}</label>
		</dd>
	</dl>
</fieldset>
